...
function pickEmailOrPhone(body) {
  const emailOrPhone = toStr(body.emailOrPhone || body.target);
  if (emailOrPhone) {
...
export async function login(req, res) {
  try {
    ...
    if (!user.passwordHash) return res.status(400).json({ message: "No local password set" });
...
export async function requestOtp(req, res) {
  try {
    ...
    // ✅ During local debugging, show the real reason
    if (!isProd()) {
...
export async function verifyOtp(req, res) {
  try {
    ...
    if (!user || !user.otp || !user.otp.code) return res.status(404).json({ message: "OTP not found" });
...
export async function clerkLogin(req, res) {
  try {
    ...
    console.error("clerkLogin error:", error);
    if (error && error.code === 11000) return res.status(409).json({ message: "User exists" });
    return res.status(500).json({ message: "Internal server error" });
  }
}
